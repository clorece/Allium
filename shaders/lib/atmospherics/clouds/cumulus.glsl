float GetCumulusDetail(vec3 pos, vec3 offset, float persistence) {
    ...
    for (int i = 0; i < detailSamples; ++i) {
        ...
        //    n *= Noise3D(p * (8.0 + float(i) * 1.5) + offset * 1.5);
        detail += n * amplitude;
        total += amplitude;
...
float GetCumulusCloud(vec3 tracePos, int steps, int cloudAltitude, float lTracePosXZ, float cloudPlayerPosY, float noisePersistence, float mult, float size) {
    ...
    float base = Noise3D2(tracePosM * 3.5 + offset) * 12.0;
    ...
    float combined = mix(base, base * detail, 1.0);
    ...
    float fadeBottom = smoothstep(cumulusCloudStretch * 0.8, cumulusCloudStretch, y - (cloudAltitude - cumulusCloudStretch));
