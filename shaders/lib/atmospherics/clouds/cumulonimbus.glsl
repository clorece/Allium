float lowerLayerCloudSizeMult = 1.0;
...
float GetCumulonimbusDetail(vec3 pos, vec3 offset, float persistence) {
    ...
    for (int i = 0; i < detailSamples; ++i) {
        ...
        float n = Noise3D(p * (8.7 + float(i) * 1.5) / lowerLayerCloudSizeMult + offset * 1.5 + windOffset);
        detail += n * amplitude;
        total += amplitude;
...
float GetCumulonimbusCloud(vec3 tracePos, int steps, int cloudAltitude, float lTracePosXZ, float cloudPlayerPosY,
    ...
    float shearAmount = 0.5; // adjust to control wind distortion
    ...
    base += Noise3D(tracePosM * 2.85  / lowerLayerCloudSizeMult + offset) * 6.0;
    base /= 8.0 / CUMULONIMBUS_COVERAGE;
    base += rainFactor * 0.7;
    ...
    float fadeTop    = smoothstep(0.0, cumulonimbusCloudStretch * 0.5, cloudAltitude + cumulonimbusCloudStretch - tracePos.y);
