...
void NeighbourhoodClamping(vec3 color, inout vec3 tempColor, float z0, float z1, inout float edge) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        #ifdef TRUE_TAAU
             ivec2 inputNeighbour = ivec2(vec2(texelCoordM2) * RENDER_SCALE);
             float z0Check = texelFetch(depthtex0, inputNeighbour, 0).r;
             float z1Check = texelFetch(depthtex1, inputNeighbour, 0).r;
        #else
        ...
        if (max(abs(GetLinearDepth(z0Check) - GetLinearDepth(z0)), abs(GetLinearDepth(z1Check) - GetLinearDepth(z1))) > 0.09) {
            ...
            #ifdef TRUE_TAAU
                if (int(texelFetch(colortex6, inputNeighbour, 0).g * 255.1) == 253)
            #else
        ...
        #ifdef TRUE_TAAU
            vec3 clr = RGBToYCoCg(texelFetch(colortex3, inputNeighbour, 0).rgb);
        #else
...
void DoTAA(inout vec3 color, inout vec3 temp, float z1) {
    #ifdef TRUE_TAAU
        // Correctly sample material mask from low-res input
    ...
    #ifndef TRUE_TAAU // Disable optimization for TAAU since we are filling the native viewport
    #if RENDER_SCALE < 1.0
    ...
    #if defined(TRUE_TAAU)
        vec2 texCoord01 = texCoord; // Already native 0-1
    #elif RENDER_SCALE < 1.0
    ...
    #ifdef TRUE_TAAU
        vec2 historyCoord = (texCoord01 + velocity01); // History is NATIVE
    #else
