...
void NeighbourhoodClamping(vec3 color, inout vec3 tempColor, float z0, float z1, inout float edge) {
    ...
    ivec2 inputCoord = ivec2(vec2(texelCoordM1)); 
    ...
    for (int i = 0; i < 8; i++) {
        ...
        ivec2 inputNeighbour = ivec2(vec2(texelCoordM2));
        ...
        if (max(abs(GetLinearDepth(z0Check) - GetLinearDepth(z0)), abs(GetLinearDepth(z1Check) - GetLinearDepth(z1))) > 0.09) {
            ...
            if (int(texelFetch(colortex6, texelCoordM2, 0).g * 255.1) == 253) edge *= extraEdgeMult;
        }
...
void DoTAA(inout vec3 color, inout vec3 temp, float z1) {
     int materialMask = int(texelFetch(colortex6, ivec2(vec2(texelCoord)), 0).g * 255.1);
    ...
    vec2 texCoord01 = texCoord / RENDER_SCALE;
